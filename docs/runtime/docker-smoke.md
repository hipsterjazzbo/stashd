...
- `podcast` rebuild, public `GET /b/{broadcastToken}/feed.xml`, public episode media route (full fetch + a `Range` request → `206`), and a non-revealing `404` for an unknown item token
