...
**Status:** in progress. Slices 1-5 shipped (toolchain/shell, auth, Dashboard/Activity,
Stashes/Vault, Create Stash + Settings). Only the cross-cutting items below are left
in this phase.
...
- [ ] True low-latency SSE streaming over RoadRunner — `TempestPsr7Bridge::toPsr7()` (`app/System/RoadRunner/TempestPsr7Bridge.php`) now drains `EventStream`'s generator and sends it as one burst once the poll loop (`EventsController::MAX_ITERATIONS`) completes, fixing the prior bug where it silently dropped the `Generator` body and sent `Content-Length: 0`. The burst-delivery fix is enough for the Dashboard/Activity/Stash-detail UI (auto-reconnect every ~MAX_ITERATIONS seconds). Real incremental push needs RoadRunner's `chunkSize`-based stream mode (`PSR7Worker::respond()`) plus a custom `StreamInterface` adapter pulling from the generator — bigger change, touches shared runtime behavior, deferred.
  ...
  - **Slice 5 added a 4th consumer, deliberately bounded rather than perpetual.** The Create Stash wizard (`src/main.entrypoint.ts`'s `awaitSseTerminal`) opens an `EventSource` only during its two short wait steps (preflight discovery, stash creation) and explicitly `.close()`s it once the watched command reaches a terminal state — not a page-lifetime subscription that auto-reconnects forever like Dashboard/Activity/Stash-detail. Per-connection cost is unchanged (the loop still runs its full `MAX_ITERATIONS` once opened, regardless of how soon the client closes), but total exposure is roughly two ~10s cycles per stash created, not an indefinite drain for as long as a tab stays open — so it doesn't count toward the "how many tabs can be open at once" threshold above the same way a 4th perpetual page would. **The next page that wants a perpetual live-update subscription (not a short bounded wait) is still the one that should trigger the real fix above or a server-side connection cap — not another `num_workers` bump.**
