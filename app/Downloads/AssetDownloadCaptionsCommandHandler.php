...
final readonly class AssetDownloadCaptionsCommandHandler implements CommandHandler
{
    ...
    public function type(): CommandType { return CommandType::AssetDownloadCaptions; }
    public function validate(array $options): void
    {
        $id = is_string($options['media_item_id'] ?? null) ? $options['media_item_id'] : '';
        if (! MediaItemId::isValid($id) || $this->mediaItems->find(MediaItemId::parse($id)) === null) throw InvalidCommandPayload::withErrors(['Media item not found.']);
    }
    public function createJobs(CommandRecord $command, array $options): array
    {
        ...
        $command->options = $payload; $command->targetType = 'media_item'; $command->targetId = $payload['media_item_id']; $this->commands->save($command);
        $entityId = PrefixedUlid::parse($payload['media_item_id']);
        if ($this->jobs->hasPendingOrProcessing(JobIntent::DownloadCaptions, $entityId)) return [];
    ...
    public function extras(CommandRecord $command, array $options): array { return []; }
}
