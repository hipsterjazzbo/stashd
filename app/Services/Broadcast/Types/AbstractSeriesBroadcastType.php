...
use App\Domain\Broadcast\BroadcastPlannedFile;
use App\Domain\Broadcast\BroadcastPlannedSidecar;
use App\Domain\Broadcast\BroadcastPruneResult;
use App\Domain\Broadcast\BroadcastPublishResult;
use App\Domain\Broadcast\BroadcastSidecarKind;
...
use App\Infrastructure\Persistence\BroadcastItemRepository;
use App\Infrastructure\Persistence\RecordTimestamps;
use App\Services\Broadcast\BroadcastContextFactory;
use App\Services\Broadcast\BroadcastFilenameBuilder;
use App\Services\Broadcast\BroadcastNfoBuilder;
use App\Services\Broadcast\BroadcastPathBuilder;
use App\Services\Broadcast\BroadcastSidecarWriter;
use App\Services\Broadcast\HardlinkPublisher;
use App\Services\State\StateTransitionService;
use RecursiveDirectoryIterator;
use RecursiveIteratorIterator;
use SplFileInfo;

abstract readonly class AbstractSeriesBroadcastType implements BroadcastTypeHandler
{
    ...
    abstract protected function profile(): SeriesBroadcastProfile;
    ...
    public function publish(BroadcastContext $context, BroadcastPlan $plan): BroadcastPublishResult
    {
        ...
        foreach ($plan->files as $planned) {
            ...
            try {
                ...
                $publishedCount++;
            } catch (\App\Domain\Broadcast\BroadcastException $exception) {
    ...
    private function publishPosterSidecar(BroadcastPlannedSidecar $sidecar): void
    {
        ...
        } catch (\App\Domain\Broadcast\BroadcastException) {
    ...
    private function upsertHardlinkAsset(
        ...
        $existing = \App\Domain\Media\AssetRecord::select()
    ...
    private function markItemStale(\App\Domain\Broadcast\BroadcastItemRecord $item, string $reason): void
    {
